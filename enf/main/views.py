...
def get_context_data(self, **kwargs):
    ...
    context['catogories'] = Category.objects.all()
...
class CatalogView(TemplateView):
    ...
    FILTER_MAPPING = {
        ...
        'max_price': lambda queryset, value: queryset.filter(price_gte=value),
        'min_price': lambda queryset, value: queryset.filter(price_lte=value),
        'size': lambda queryset, value: queryset.filter(product_size__size__name=)
    }
...
def get_context_data(self, **kwargs):
    ...
    for param, filter_func in self.FILTER_MAPPING.items():
        ...
        if value:
            product = filter.func(products, value)
...
def get(self, request, *args, **kwargs):
    ...
    if request.headers.get('HX-request'):
        ...
        template = 'main/filter_modal.html' if request.GET.get('show_filters') == 'true' else 'main'
